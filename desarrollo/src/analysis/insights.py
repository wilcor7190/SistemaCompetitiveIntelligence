...
Uses Ollama (qwen3.5:9b) if available, falls back to pandas statistics.
"""
...
class InsightGenerator:
    ...
    async def generate_insights(self, df: pd.DataFrame) -> list[Insight]:
        ...
        Falls back to statistics-based insights if Ollama unavailable.
        """
        if df.empty:
            self.logger.warning("[insights] Empty DataFrame, no insights")
            return []

        # Always generate stats-based insights (reliable, no LLM needed)
    ...
    def _insight_pricing(self, df: pd.DataFrame) -> Insight:
        ...
        if len(avg_by_platform) >= 2:
            delta_pct = ((avg_by_platform.max() - avg_by_platform.min()) / avg_by_platform.min() * 100)
    ...
    def _insight_operational(self, df: pd.DataFrame) -> Insight:
        ...
        coverage_str = ", ".join(f"{k}: {v} productos" for k, v in total_products.to_dict().items())
    ...
    def _insight_promotions(self, df: pd.DataFrame) -> Insight:
        ...
        # Count by platform
        by_platform = promo_data.groupby("platform").size().to_dict() if not promo_data.empty else {}
        promos_str = ", ".join(f"{k}: {v}" for k, v in by_platform.items()) or "sin datos"
    ...
    def generate_executive_summary(self, df: pd.DataFrame, insights: list[Insight]) -> str:
